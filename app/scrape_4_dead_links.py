			...
			for current_url, status in my_list.iteritems():
				...
				with open('app/templates/logs.html','a') as out:
					out.write ('{}'.format(current_url))
																												# Use wgets spider to check all links
																												# -t (3s) time to wait if link doesnt respond,
																												# remove check for robots.txt,
																												# -r recurse website. 
																												# Output results to a list.
				output_results = commands.getoutput('wget --spider -t 3 -e robots=off -r -p ' + current_url)
				list_of_results = output_results.split('\n')
				
				dead_link_array = []
				for item in list_of_results:
																												# Check list for 404, this is found at end of string.
																												# If found store bad link in array which is 
																												# 3 positions before the 404 is found.
					current_item_pos = current_item_pos + 1
					if item[-13:] == '404 Not Found':
						no_of_four_o_four = no_of_four_o_four + 1 
						print list_of_results[current_item_pos-3]
						print item
						dead_link_array.append(list_of_results[current_item_pos-3])
				...
				if ((no_of_four_o_four > 0) and (status == 'OK')):
					my_list[current_url] = dead_link_array
				
				if (no_of_four_o_four == 0):
					my_list[current_url] = "OK"

				print "Number of Dead Links: %d" %no_of_four_o_four
			...
			if ( dead_link_found == False ):
				with open('app/templates/check.xml','w') as out:
					...
					out.write('{}\n{}\n{}\n{}\n{}\n'.format(xml_out_1,xml_out_2,xml_out_3,xml_out_4,xml_out_5))
																												# Sleep for duration of check interval.
